...
function Header() {
  ...
  const checkOut = async () => {
    const stripe = await loadStripe(import.meta.env.VITE_PUBLISHED_KEY);
    const res = await axios.post(`${backendURL}/checkOut`, {
      userID: user.id,
      data: data.data,
    });
              ...
              <DialogContent className="top-0 h-full right-0 max-w-[400px]">
                ...
                <SignedIn>
                  ...
                  <div className="checkout bg-white flex gap-2 mt-3 flex-col fixed bottom-5">
                    ...
                    <Button className="capitalize" onClick={checkOut}>
                      check out
                    </Button>
                  </div>
                </SignedIn>
